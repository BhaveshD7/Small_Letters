...
// import { fetchPostBySlug, likePost, savePost } from '../api/posts'
...
//     if (liked) return
//     try { const r = await likePost(post._id); setLikeCount(r.data.likes); setLiked(true) } catch {}
//   }
//   const handleSave = async () => {
//     if (saved) return
//     try { await savePost(post._id); setSaved(true) } catch {}
//   }
...
//           <button className={`pill ${liked ? 'pill-active' : ''}`} onClick={handleLike}>
//             <svg viewBox="0 0 24 24" fill={liked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
//               <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>
//             </svg>
//             {likeCount}
//           </button>
//           <button className={`pill ${saved ? 'pill-active' : ''}`} onClick={handleSave} title="Save">
...
export const getAllSeries = () =>
  axios.get(`${API_URL}/api/posts/series/all`, getAuthHeader());
...
export default function Post() {
  ...
  const checkUserInteractions = async (postId) => {
    try {
      const res = await axios.get(
        `${API_URL}/api/interactions/check/${postId}`,
        getAuthHeader()
      )
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/interactions/like/${post.id}`,
        {},
        getAuthHeader()
      )
      setLiked(res.data.liked)
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/interactions/save/${post.id}`,
        {},
        getAuthHeader()
      )
      setSaved(res.data.saved)
