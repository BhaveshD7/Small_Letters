...
// import { fetchPosts, fetchPopularPosts } from '../api/posts'
...
//   useEffect(() => {
//     Promise.all([fetchPosts(1, 10), fetchPopularPosts()])
//       .then(([p, pop]) => { setPosts(p.data.posts); setPopular(pop.data.posts) })
//       .catch(console.error)
...
//           {/* {leftPosts.map(p => <PostCard key={p.id} post={p} variant="small" />)} */}
...
//           {/* All remaining posts in small card format */}
...
//                   <Link to={`/post/${p.slug}`} className="grid-post-title">{p.title}</Link>
//                   {p.subtitle && <p className="grid-post-subtitle">{p.subtitle}</p>}
...
//                     <span>·</span>
//                     <span>{p.author?.name || 'CARINA'}</span>
//                   </div>
//                 </div>
//               </div>
//             ))}
...
//             <p className="home-subscribe-hed">words that feel like yours.</p>
//             <p className="home-subscribe-sub">Get every new piece in your inbox.</p>
...
//         {popular.map(p => <PostCard key={p.id} post={p} variant="popular" />)}

//         <div className="sidebar-divider" />
//         <div className="sidebar-head">Browse by type</div>
//         <div className="type-filters">
//           {['quote', 'phrase', 'paragraph', 'essay'].map(t => (
//             <Link key={t} to={`/archive?type=${t}`} className="type-chip">{t}</Link>
//           ))}
//         </div>

//         {/* ADD THIS NEW SUBSCRIBE CARD */}
//         <div className="sidebar-divider" />
